...
class Main{
	...
	static void setHLD(int nowNode, int level) {
		...
		for(int i=0; i<adNode[nowNode].size(); i++)
		{
			...
			else		// light인 경우, 새로운 체인 시작
			{
				...
				setHLD(child, level + 1);		// 레벨을 늘려 새로운 체인 시작
			}
			update(1, 1, N, treeIdx[child], 1);// 1번 노드는 부모가 없으므로 간선 값을 0으로 세팅
		}
	}
	static void moveHeavyChildToFront(int nowNode, int[] size) {
		int heavySize = 0;
		int heavyIdx = 0;
		size[nowNode] = 1;
		for(int i=0; i<adNode[nowNode].size(); i++)
		{
			...
			moveHeavyChildToFront(child, size);
	...
	public static void main(String[] ags)throws Exception{
		...
		visit[1] = true;
		moveHeavyChildToFront(1, new int[N + 1]);
		...
		while(Q-->0)
		{
			...
			{
				int sum = query(1, 1, N, treeIdx[chainHead[node2]], treeIdx[node2]);
				int ex = treeIdx[node2] - treeIdx[chainHead[node2]] + 1;// 예상 결과 값, 자기와 자기 부모노드 사이 간선 개수 카운팅
				if(sum == ex)	// 노드가 연결되어있는게 맞다면
			...
			{
				int sum = query(1, 1, N, treeIdx[chainHead[node1]], treeIdx[node1]);
				int ex = treeIdx[node1] - treeIdx[chainHead[node1]] + 1;// 예상 결과 값, 자기와 자기 부모노드 사이 간선 개수 카운팅
				if(sum == ex)	// 노드가 연결되어있는게 맞다면
					node1 = chainParent[node1];
				else
					isPossible = false;
				
				int sum1 = query(1, 1, N, treeIdx[chainHead[node2]], treeIdx[node2]);
				int ex1 = treeIdx[node2] - treeIdx[chainHead[node2]] + 1;// 예상 결과 값, 자기와 자기 부모노드 사이 간선 개수 카운팅
				if(sum1 == ex1)	// 노드가 연결되어있는게 맞다면
					node2 = chainParent[node2];
				else
					isPossible = false;
			}
			...
			if(isPossible)
			{
				...
				int ex = treeIdx[node2] - treeIdx[node1];
				int sum = query(1, 1, N, treeIdx[node1] + 1, treeIdx[node2]);
				if(ex != sum)
					isPossible = false;
			}
			...
			if(d == 1)// d가 1이면
			{
				...
				else			// b와 c를 연결하는 통로가 없으면 c의 부모정점과 c를 연결하는 엣지 제거
				{
					deleteNode = c;
				}
				update(1, 1, N, treeIdx[deleteNode], 0);
			}
