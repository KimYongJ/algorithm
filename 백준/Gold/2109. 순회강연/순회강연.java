...
class Main{
	...
	public static int getParent(int parent[], int idx) {
		...
		return parent[idx] = getParent(parent, parent[idx]);// 경로 압축 기법, union find에서 find 효율을 높이기 위해 사용, 검색 경로상 모든 노드들이 직접 최종 루트를 가리키도록 업데이트함으로 속도 향상
	}
	
	public static void main(String[] args)throws Exception{
		PriorityQueue<Node> pq = new PriorityQueue<Node>((a,b)->b.money-a.money);
		...
		while(!pq.isEmpty())
		{
			...
			int parentNode = getParent(parent,now.day);	// 현재 큐의 날짜로 부터 0과 가장 가까운 날을 가져옴 
