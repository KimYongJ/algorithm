...
class Main{
	...
	public static Node[][][] copy(Node[][][] o) {
		Node[][][] result= new Node[N][4][4];
		
		for(int n=0; n<N; n++)
			for(int y=0; y<4; y++)
				for(int x=0; x<4; x++)
					result[n][y][x] = new Node(o[n][y][x].value, o[n][y][x].ch);
	...
	public static Node[][] INPUT(Node[][] nextResult, int idx, int Y, int X, int dir){
		for(int y=0; y<4; y++)
			for(int x=0; x<4; x++)
			{
				nextResult[y+Y][x+X].value += origin[dir][idx][y][x].value;
