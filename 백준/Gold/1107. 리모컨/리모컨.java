...
// DFS의 인자 : 깊이, 만들어진 자릿수들의정보 , 가능한숫자배열(arr), 최대 깊이
...
class Main{
	...
	static int min = Integer.MAX_VALUE; // 최소 움직임 값
	...
	public static void main(String[] args)throws Exception{
		...
		for(int i=0; i<t; i++) {
			nList[ Integer.parseInt(st.nextToken()) ] = true;
		}
		
		int arr[] = new int[10-t], idx = 0;
		...
		if(goalInt == 100) { // 목표가 100이면 100출력
			...
			System.out.print(Math.abs( goalInt - 100 ));
			return;
		}
		int i = goalLength == 1 ? goalLength : goalLength-1;
		for(; i<=goalLength+1; i++)
			DFS(0,0,arr,i); // 숫자 c 만들기 

        min = Math.min(min, Math.abs(goalInt-100));
        
		System.out.println(min);
	}
	public static void DFS(int depth,int make,int[] arr,int max) {
		if(depth==max) {
			// 최소 이동 값을 바로 min으로 저장.
			int len = String.valueOf(make).length(); // 해당 숫자를 누르는데까지 걸리는 클릭수이며 100일 경우 길이는 0
			int value = Math.abs(make-goalInt) + len;// 만든숫자와 목표숫자를 빼고 길이를 더해서 이동횟수를 구한다.
			if(value<min)
				min = value;
			return;
		}
		for(int i=0; i<arr.length; i++) {
			DFS(depth+1, make*10+arr[i],arr,max);
		}
	}  
}