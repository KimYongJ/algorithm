...
public class Main {
	...
	static long money, cost[];
	static int N, M, start, end;
	static boolean visit[];
	static PriorityQueue<Node> pq;
	static ArrayList<Node>[] adlist;
	public static void Dijkstra() {
		cost[start] = 0;					// start까지 가는데 든 지불비용 0
		...
		while(!pq.isEmpty()) {
			...
			for(Node next : adlist[now.node]) 
			{
				...
				if(now.money >= next.money) 			// 돈을 지불해야 하기에 현재 갖고있는 돈이 지불할 비용(next.money)보다 커야함
				{
					if(cost[next.node] > next.money) 	// 가려는 노드까지 도달할 때 드는 지불비용이 최소여야 다음 노드로 갈 수 있음
					{
						cost[next.node]= Math.max(now.maxMoney,next.money); // 해당 노드까지 방문하는데 든 가장 큰 지불비용 삽입
						pq.add(new Node(next.node, now.money - next.money, cost[next.node]));
					}
				}
	...
	public static void main(String[] args)throws Exception{
		...
		cost	= new long[N+1];							// 해당 노드 까지 가는데 든 가장 적은 지불비용
