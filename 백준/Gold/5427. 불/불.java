...
class Main{
	...
	public static void BFS() {
		...
		while(!q.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(x>=0 && y>=0 && x<X && y<Y && map[y][x] !='#') {
					if(now.isfire && map[y][x] != '*') { // 불일 경우
						map[y][x] = '*';
						q.add(new Node(y, x, 0, true));
					}else if(!now.isfire && map[y][x] == '.') { // 불이 아니고 좌표가 이동가능할 경우
						q.add(new Node(y,x, now.time +1, false));
						map[y][x] = '*'; // 상근이가 방문해서 큐에 넣은 곳은 불로처리해 두번다시 방문하지 못하게 합니다. 
					}
				}
			}
		...
		// 여기까지 왔다는 것은 상근이가 테두리에 못도달했다는 것이다.
		...
		while(T-->0) {
			...
			for(int y=0; y<Y; y++) {
				...
				for(int x=0; x<X; x++) {
					char c = str.charAt(x);
					map[y][x] = c;
					if(c=='*') { // 불일 경우 큐에 먼저 넣어 먼저 연산되도록 한다.
						q.add(new Node(y, x, 0, true));
					}else if(c=='@') { // 상근이일 경우 마지막에 큐에 넣어야 한다 빠른 연산을 위해 상근이자리도 불로처리해서 방문 하지 못하도록함 
						startY = y;
						startX = x;
						map[y][x] = '*';
					}
