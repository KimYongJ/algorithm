...
class Main{
	...
	public static int makeNumber(int i, int j, int num) { 
		...
		num 		-= typeI * digitI;					// 구한 자릿수를 빼고 더해 서로 바꾸는 과정.						
		num 		-= typeJ * digitJ;					// 구한 자릿수를 빼고 더해 서로 바꾸는 과정.
		num 		+= typeI * digitJ;					// 구한 자릿수를 빼고 더해 서로 바꾸는 과정.
		num 		+= typeJ * digitI;					// 구한 자릿수를 빼고 더해 서로 바꾸는 과정.
		if(num < base) { 								// 자릿 수 교환시 맨 앞이 0일 경우
			num = 0;
		}
		return num;
	}
	// DFS 함수, 각 숫자를 0번째부터 마지막 전까지 순회하면서 전체 다 대입해봄
	public static void DFS(int start, int k) {
		if(k == K || start==0) { 						// 연산을 k번 했을 경우 값 확인 후 종료
		...
		for(int i=0; i<digit-1; i++) 
		{
			...
			{
				START = makeNumber(i, j, start);// 새로운 숫자 생성
				if(!visit[k][START])// k+1번째에 해당 숫자가 있는지 체크 
				{
					visit[k][START] = true;		// 없다면 set, q에 추가
					DFS(START, k);
				}
			}
		}
