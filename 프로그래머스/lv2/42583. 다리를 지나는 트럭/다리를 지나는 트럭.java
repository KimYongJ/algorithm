...
class Solution {
    ...
    public int result = 0; // 최종 시간을 반환할 변수 
    ...
    public int solution(int bridge_length, int weight, int[] truck_weights) {
        ...
        while(idx<truck_weights.length){ // 대기트럭의 길이보다 순회할 인덱스가 작을 때만 반복

            int truck = q.poll(); // 큐에서 데이터를 하나 뽑아온다.
            
            if(truck != 0){ // 트럭의 무게 측정, 0이 아닌 경우 이하 코드 실행
                weightSum-= truck; // 다리를 건너는 중인 트럭무게 합에서 방금 큐에서 뽑은 트럭 무게를 뺀다
            }
